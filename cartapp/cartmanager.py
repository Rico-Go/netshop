    ...
    def migrateSession2DB(self):
        if 'user' in self.session:
            ...
            for cartitem in self.queryAll():
                if user.cartitem_set.filter(goodsid=cartitem.goodsid,colorid=cartitem.colorid,sizeid=cartitem.sizeid).count()==0:
                    cartitem.user = user
                    cartitem.save()
                else:
                    item = user.cartitem_set.get(goodsid=cartitem.goodsid,colorid=cartitem.colorid,sizeid=cartitem.sizeid)
...
class DBCartManger(CartManager):
    ...
    def queryAll(self,*args,**kwargs):
        ...
        # print(self.user.cartitem_set.order_by('id').filter(isdelete=False).all())
